...
async def login_for_access_token(
    ...
    # Looking up user by their email (case insensitive)
    # Note: OAuth2PasswordRequestForm uses 'username' field which can be utilised for email
    result = await db.execute(
        select(models.User).where(
    ...
    # verify user existence and their password
    # Note: Never reveal which failed ( security best practices )
    ...
    # creating access token with user id 
...
async def upload_profile_picture(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorised to update this user's picture",
        )
...
async def delete_user_picture(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorised to delete this user's picture",
        )
